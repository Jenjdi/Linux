...
// {
//     int pos=out.find(sep);
//     if(pos==out.npos)
//     {
//         return std::string();
//     }
//     int len=std::stoi(out.substr(0,pos));
//     int total=std::to_string(len).size()+sep.size()*2+len;
//     std::string jsonstream=out.substr(pos,len);
//     if(jsonstream.size()<total)//收到不完整的报文
//     {
//         return std::string();
//     }
//     out.erase(total);
